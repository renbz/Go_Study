...
// 定义一个HeroNode
type HeroNode05 struct {
	no       int
	name     string
	nickname string
	pre      *HeroNode05 //这个表示指向前一个结点
	next     *HeroNode05 //这个表示指向下一个结点
}

// 给双向链表插入一个结点
// 编写第一种插入方法，在单链表的最后加入.[简单]
func InsertHeroNode05(head *HeroNode05, newHeroNode *HeroNode05) {
	//思路
	//1. 先找到该链表的最后这个结点
	//2. 创建一个辅助结点[跑龙套, 帮忙]
	temp := head
...
func InsertHeroNode205(head *HeroNode05, newHeroNode *HeroNode05) {
	//思路
	//1. 找到适当的结点
	//2. 创建一个辅助结点[跑龙套, 帮忙]
	temp := head
...
// 删除一个结点[双向链表删除一个结点]
func DelHerNode05(head *HeroNode05, id int) {
	temp := head
	flag := false
...
// 显示链表的所有结点信息
// 这里仍然使用单向的链表显示方式
func ListHeroNode05(head *HeroNode05) {
...
func ListHeroNode2(head *HeroNode05) {
...
func main() {
	...
	InsertHeroNode05(head, hero1)
	InsertHeroNode05(head, hero2)
	InsertHeroNode05(head, hero3)
	ListHeroNode05(head)
	fmt.Println("逆序打印")
	ListHeroNode2(head)
